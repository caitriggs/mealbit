---
title: White bean smash toast with chili oil
kind: lunch
lunch_style: assembly
makes: 2
serves: 2
active_time: 5
total_time: 5
seasons: [deep-winter, early-spring, late-spring, summer, early-fall, late-fall]
protein: legume
cuisine: italian
effort: easy
register: elegant
tags: [five-minutes, vegetarian, pantry-only, emergency-lunch]
ingredients:
  - 1 bunch parsley || basil in summer [produce]
  - 1 can cannellini beans [pantry]
  - 1 loaf sourdough [bakery]
  - 1 jar chili onion crunch || any chili oil [specialty]
  - 1 wedge Parmigiano-Reggiano [dairy]
  - 1 lemon [produce]
  - 1 tomato (summer only) || jarred sun-dried tomatoes [produce]
pantry: [olive oil, garlic, chili flakes, kosher salt, black pepper, red wine vinegar, capers]
per_plate: [tomatoes]
photo_url: https://images.pexels.com/photos/5150301/pexels-photo-5150301.jpeg?auto=compress&cs=tinysrgb&h=650&w=940
photo_credit: ROMAN ODINTSOV / Pexels
photo_source: https://www.pexels.com/photo/cooked-food-on-white-ceramic-plate-5150301/
photo_pinned: true
rating: null
---

**Why it's on the list:** the emergency lunch. Every ingredient lives in the cupboard
permanently, so this is the answer to "there's nothing in the house" that isn't
DoorDash.

**The move:** **rub the hot toast with a raw garlic clove.** The toast acts as a grater;
the clove melts into the surface. It takes four seconds and it is the entire difference
between beans on toast and something you'd order.

1. Thick sourdough, toasted hard — properly dark, or it collapses under the beans.
2. **Rubbed with a halved raw garlic clove while still hot**, then flooded with olive
   oil and salt.
3. Beans drained, smashed roughly with a fork with 2 tbsp olive oil, lemon juice and
   zest, 1 tsp vinegar, a pinch of chili flakes, salt, pepper. Leave it lumpy.
4. Piled on. Chili crisp, shaved Parmigiano, parsley, more olive oil than seems
   reasonable. Tomato if it's August.

**Per plate:** the smash is lemon, olive oil and chili — complete on its own. August tomato goes on the toast that wants it only.
